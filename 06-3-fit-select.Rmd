...
```{r}
printCoefmat(anova(nest, comp), na.print = "")
```
...
```{r}
printCoefmat(anova(nest, comp), signif.legend = F, na.print = "")
```
...
```{r fpredB, fig.ncol=2, fig.subcap = c('', '', '', ''), out.extra="", fig.show='hold', out.width="50%", out.height="50%", fig.scap="Biomass predictions by different tables fitted to data from 42 trees measured in Ghana by @henry10", fig.cap="(ref:fpredB)"}
...
with(dat[dat$Btot>0,], plot(dbh, Btot, xlab="Dbh (cm)", ylab="Biomass (t)", log="xy"))
D <- 10^par("usr")[1:2]
...
```{r fselm, fig.scap="Biomass predictions by the same power table fitted in three different ways to data from 42 trees measured in Ghana by Henry et al. 2010", fig.cap="(ref:fselm)"}
