...
\@ref(eq:icind) for a tree selected at random and the bounds of confidence interval \@ref(eq:icpeup) for the mean tree is approximately $t_{n-2}\hat{\sigma}$. This difference reflects the difference between equations \@ref(eq:Yind) and \@ref(eq:Ypeup), which is due to the residual term $\varepsilon$ whose estimated standard deviation is $\hat{\sigma}$.
...
(@eq-ficlnD) (ref:ficlnD)

:::{.exercise #ficlnD name="(ref:fic:lnD)"}
\  
:::
...
(@eq-ficlnDH) (ref:ficlnDH)

:::{.exercise #ficlnDH name="(ref:ficlnDH)"}
\  
:::
...
predict(m,newdata=data.frame(dbh=20,heig=20),interval="confidence",level=0.95)
```
...
```{r, echo=T, results='hide'}
predict(m,newdata=data.frame(dbh=20,heig=20),interval="prediction",level=0.95)
```
...
where $q$ is the number of coefficients in the model (i.e. the length of vector $\theta$), $\mathrm{d}_{\theta}f(\hat{\theta})$ is the value in $\theta=\hat{\theta}$ of the differential of $f$ respect to model coefficients, and $\hat{\boldsymbol{\Sigma}}_{\hat{\theta}}$ is an estimation in $\theta=\hat{\theta}$ of the variance-covariance matrix 
...
1. For $k$ of $1$ to $Q$:
    + 1.1 draw a vector $\hat{\theta}^{(k)}$ following a multinormal distribution of mean $\hat{\theta}$ and variance-covariance matrix $\hat{\boldsymbol{\Sigma}}$;
    + 1.2 calculate the prediction $\hat{Y}^{(k)}=f(X_1,\ \ldots,\ X_p;\hat{\theta}^{(k)})$.
...
1. For $k$ of $1$ to $Q$:
    + 1.1. draw a vector $\hat{\theta}^{(k)}$ following a multinormal distribution of mean $\hat{\theta}$ and variance-covariance matrix $\hat{\boldsymbol{\Sigma}}$;
    + 1.2. draw a residual $\hat{\varepsilon}^{(k)}$ following a centered normal distribution of standard deviation $\hat{\sigma}$ (linear case) or $\hat{k}X_1^{\hat{c}}$ (non-linear case);
    + 1.3. calculate the prediction $\hat{Y}^{(k)}=f(X_1,\ \ldots,\ X_p;\hat{\theta}^{(k)})+\hat{\varepsilon}^{(k)}$.
...
1. For $k$ of $1$ to $Q$:
    + 1.1. for $i$ of 1 to $p$, draw $\hat{X}_i^{(k)}$ following a normal distribution of mean $X_i$ and standard deviation  $\tau_i$;
    ...
    + 1.3. draw a residual $\hat{\varepsilon}^{(k)}$ following a centered normal distribution of standard deviation $\hat{\sigma}$ (linear case) or $\hat{k}X_1^{\hat{c}}$ (non-linear case);
    + 1.4. calculate the prediction $\hat{Y}^{(k)}=f(\hat{X}_1^{(k)},\ \ldots,\ \hat{X}_p^{(k)}; \hat{\theta}^{(k)})+\hat{\varepsilon}^{(k)}$.
...
Let `m` be the object containing the fitted model (red line \ref{fdens}). The adjustment factor $\mathrm{CF}=\exp(\hat{\sigma}^2/2)$ is obtained by the command:
