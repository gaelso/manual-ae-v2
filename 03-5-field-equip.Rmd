...
- Labels (stapled onto the disks), staples and stapler; or fuschine pencil (but, if the samples are to be stored for later measurements of nutrient content, fuchsine should be avoided) (photo \ref{mats}).
- Cutters, machetes, axes, shears, saws (photo \ref{mats}).
- Iron frames for making bundling branches (photo \ref{cad}) or alternatively bins of different sizes.
- Power saws (ideally one power saw suitable for felling trees and another smaller more maneuverable power saw for limbing --- photo \ref{mats}).
...
- Very strong large bags (such as grain or sand bags --- photo \ref{tsp}) for transporting the disks and field samples to vehicles (if these are parked some distance from the felling site).
