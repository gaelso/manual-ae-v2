...
```{r cha, out.width='100%', fig.cap="Chain running from the studied stand to the variables we are looking to predict."}
...
grViz("digraph cha {
  ...
  A[label = 'Stand \nstudied', fontsize = 40]
  B[label = 'Trees \nmeasured', fontsize = 40]
  C[label = Model,  fontsize = 40]
  D1[label = 'Volume of an \nindividual tree', fontsize = 40]
  D2[label = 'Mean tree volume', fontsize = 40]
  D3[label = 'Volume of the \nstudied stand', fontsize = 40]
  D4[label = 'Volume of \nanother stand', fontsize = 40]
  
  ## Add edge statements
  A -> B [label = 'sampling', fontsize = 28]
  A -> B [label = 'plan', fontsize = 28]
  B -> C [label = 'model', fontsize = 28]
  B -> C [label = 'construction', fontsize = 28]
  C -> D1 [label = 'prediction', fontsize = 28]
  C -> D2
  C -> D3
  C -> D4 [label = '', fontsize = 28]
  C -> D4 [label = 'extrapolation', fontsize = 28]
...
```{r bad, fig.scap="Volume prediction based on a linear regression constructed using extreme points when the size-volume relationship is indeed linear and when it is not", fig.cap="(ref:bad)"}
...
If we look at equation \@ref(eq:est), we recognize it as being a type of estimator that is widely used in sampling theory: the *regression estimators*. The theoretical basis of regression estimators is described in detail in [chapter 7 @cochran77] and in [chapter 8 @thompson92]. A description of regression estimators used in forestry is given in @vries86 and in [chapter 6 @shiver96] (the first is more theoretical, the second more applied). The theory of regression estimators applies in the case of a linear relationship between the quantity to be predicted ($\bar{V}$ in the example above) and an auxiliary variable ($\overline{D^2H}$ in the example above). By contrast, this theory is less well developed in the case of non-linear relations or multiple regressions, even though such cases often arise in connection with volume tables.

The semi-amplitude of the confidence interval at $\alpha$ (typically 5\ \%) of $\bar{V}$ is [p.199 @cochran77; p.83 @thompson92]:
