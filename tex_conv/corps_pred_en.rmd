...
that was fitted in red line \@ref(exr:rllnBvD). Let
...
and $\ln(H)$ that was fitted in red line \@ref(exr:flnDlnH).
Let `m` be the object containing the fitted model (see red line \@ref(exr:flnDlnH)). The confidence intervals may be calculated using the `predict` command. For example, for a tree with a dbh of 20~cm and a height of 20~m, the 95\ \% confidence interval for the mean tree is obtained by the command:
...
Let `m` be the object containing the fitted model (red line 
...
where `m` is the object containing the fitted model, and in this example is \decimal{1}{059859}. By way of a comparison, the previously calculated correction factor (red line \@ref(exr:CFB)) was \decimal{1}{061035}.
\end{filrouge}
