...
(*iii*) that the residuals are of constant variance:
...
A weighted regression can be used to fit a model such as (\ref{pol}) where the variance of the residuals is not constant, while nevertheless using the formalism of the linear model. It may be regarded as extending multiple regression to the case where the variance of the residuals is not constant. Weighted regression was developed in forestry between the 1960s and the 1980s, particularly thanks to the work of @cunia64; @cunia87b. It was widely used to fit linear tables [@whraton87; @brown89; @parresol99], before being replaced by more efficient fitting methods we will see in section \@ref(lme).
...
The third way of estimating $c$ consists in looking for the value of $c$ that minimizes @furnival61 index. This index is defined on page \pageref{furni}.
...
\end{tabular}}%
The model is therefore: $B=\decimal{2}{747}\times10^{-5}D^2H$, with $R^2$ = \decimal{0}{8897} and a residual standard deviation $k=\decimal{0}{0003513}$~tonnes~cm$^{-2}$. The model is highly significant (Fisher's test: $F_{1,41}=\decimal{330}{8}$, p-value $<\decimal{2}{2}\times10^{-16}$). As this model was fitted directly on non-transformed data, it should be noted that it was unnecessary to withdraw the observations of zero biomass (unlike the situation in red line \@ref(exr:rllnBvD2H)).  Figure \@ref(fig:fD2Hpres)A, obtained by the command:
...
\end{center}
\caption[Plot of standard deviation of biomass calculated in five dbh classes against median dbh of the class, for 42 trees measured in Ghana by @henry10]{Plot of standard deviation of biomass calculated in five dbh classes against median dbh of the class (on a log scale), for 42 trees measured in Ghana by @henry10.\label{fpond}}
\end{figure}
...
\end{center}
\caption[Plot of weighted residuals against fitted values for a weighted regression of biomass against $D^2H$ for 42 trees measured in Ghana by @henry10]{Plot of weighted residuals against fitted values for a weighted regression of biomass against $D^2H$ for 42 trees measured in Ghana by @henry10: (A) the weighting is inversely proportional to $D^4$; (B) the weighting is inversely proportional to $D^2$; (C) the weighting is inversely proportional to $D^5$.\label{fD2Hpres}}
\end{figure}
...
$B=-\decimal{3}{840}\times10^{-3}D+\decimal{1}{124}\times10^{-3}D^2$.
 Figure \@ref(fig:fDD2res) obtained by the command:
...
\end{center}
\caption{Plot of weighted residuals against fitted values for a weighted regression of biomass against $D$ and $D^2$ for 42 trees measured in Ghana by @henry10.\label{fDD2res}}
\end{figure}
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{1}{977736}$. Like for the weighted linear regression (red line \@ref(exr:frlpD2H)), the y-intercept proves not to be significantly different from zero. We can therefore refit the model without the intercept:
%
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{1}{980263}$. This value is very similar to that evaluated for the weighted linear regression ($c=2$ in red line \@ref(exr:frlpD2H)). The fitted model is therefore: $B=\decimal{2}{740688}\times10^{-5}D^2H$, which is very similar to the model fitted by weighted linear regression (red line \@ref(exr:frlpD2H)).
\end{filrouge}
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{2}{127509}$. Like for the weighted polynomial regression (red line \@ref(exr:fDpara)), the y-intercept proves not to be significantly different from zero. We can therefore refit the model without the intercept:
%
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{2}{139967}$. This value is very similar to that evaluated for the weighted polynomial regression ($c=2$ in red line \@ref(exr:fDpara)). 
The fitted model is therefore: $B=-\decimal{3}{319456}\times10^{-3}D+\decimal{1}{067068}\times10^{-3}D^2$, which is very similar to the model fitted by weighted polynomial regression (red line \@ref(exr:fDpara)).
\end{filrouge}
...
We saw in red line \@ref(exr:frlpD2H) that a double-entry biomass model using dbh and height corresponds to: $B=a+bD^2H+\varepsilon$ where $\mathrm{Var}(\varepsilon)\propto
...
We saw in red line \@ref(exr:fDpara) that a polynomial biomass model against dbh corresponded to: $B=a_0+a_1D+a_2D^2+\varepsilon$ where $\mathrm{Var}(\varepsilon)\propto
D^4$. By dividing each member of the equation by $D^2$, we obtain:
\[
...
The model is therefore written: $B=\decimal{2}{492}\times10^{-4}D^{2.346}$. Let us now return to the linear regression fitted to log-transformed data (red line \@ref(exr:rllnBvD)) which was written: $\ln(B)=-\decimal{8}{42722}+\decimal{2}{36104}\ln(D)$.
...
$B=\decimal{7}{885}\times10^{-5}(D^2H)^{0.9154}$. Let us now return to the linear regression fitted to log-transformed data (red line \@ref(exr:rllnBvD2H)), which was written:
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{2}{042586}$. This estimated value is very similar to that evaluated for the weighted non-linear regression ($c=2$, see red line \@ref(exr:frlpD2H)). The fitted model is therefore: $B=\decimal{8}{19}\times10^{-5}(D^2H)^{0.9122144}$, which is very similar to the model fitted by weighted non-linear regression (red line \@ref(exr:fnlsD2H)).
\end{filrouge}
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{2}{055553}$. This estimated value is very similar to that evaluated for the weighted non-linear regression ($c=2$, see red line \@ref(exr:frlpD2H)). The fitted model is therefore: $B=\decimal{1}{109}\times10^{-4}D^{1.9434876}H^{0.6926256}$, which is very similar to the model fitted by weighted non-linear regression (red line \@ref(exr:fnlsDH)).
\end{filrouge}
...
\end{tabular}}%
with an estimated value of exponent $c=\decimal{2}{099938}$. This result is very similar to that obtained by multiple regression on log-transformed data (red line \@ref(exr:flnDpol)).
\end{filrouge}
...
\begin{itemize}
\item red line \@ref(exr:fDpara) or \ref{finvD}: $B=-\decimal{3}{840}\times10^{-3}D+\decimal{1}{124}\times10^{-3}D^2$
\item red line \@ref(exr:fDD2var): $B=-\decimal{3}{319456}\times10^{-3}D+\decimal{1}{067068}\times10^{-3}D^2$
\item red line \@ref(exr:fnlsD): $B=\decimal{2}{492}\times10^{-4}D^{2.346}$
\item red line \@ref(exr:fnlmD): $B=\decimal{2}{445}\times10^{-4}D^{2.35105}$
\item red line \@ref(exr:frlpD2H) ou \ref{fH}: $B=\decimal{2}{747}\times10^{-5}D^2H$
\item red line \@ref(exr:fD2Hvar): $B=\decimal{2}{740688}\times10^{-5}D^2H$
\item red line \@ref(exr:fnlsD2H): $B=\decimal{7}{885}\times10^{-5}(D^2H)^{0.9154}$
\item red line \@ref(exr:fnlmD2H): $B=\decimal{8}{19}\times10^{-5}(D^2H)^{0.9122144}$
\item red line \@ref(exr:fnlsDH): $B=\decimal{1}{003}\times10^{-4}D^{1.923}H^{0.7435}$
\item red line \@ref(exr:fnlmDH): $B=\decimal{1}{109}\times10^{-4}D^{1.9434876}H^{0.6926256}$
\end{itemize}
...
and \ref{fnlmDH}). The predictions made by these different models are shown in  Figure \@ref(fig:fpredB). Let `m` be one of the fitted models with dbh as the only entry. A plot of the predictions made by this model may be obtained as follows:
...
(\ifelsecoul{magenta}{dashes and dots}). (C) Table corresponds to red line \@ref(exr:fnlsDH). (D) Table corresponds to red line \@ref(exr:fnlmDH).\label{fpredB}}
\end{figure}
...
line          &          & method     & R             &                         \\\hline%
\ref{fDpara}  & $D$      & WLS        & `lm`   & \decimal{76}{71133}     \\ %
\ref{fDD2var} & $D$      & ML         & `nlme` & \decimal{83}{09157}     \\ %
\ref{fnlsD}   & $D$      & WLS        & `nls`  & \decimal{24\ 809}{75727}\\ %
\ref{fnlmD}   & $D$      & ML         & `nlme` & \decimal{75}{00927}     \\ %
\ref{frlpD2H} & $D^2H$   & WLS        & `lm`   & \decimal{65}{15002}     \\ %
\ref{fD2Hvar} & $D^2H$   & ML         & `nlme` & \decimal{69}{09644}     \\ %
\ref{fnlsD2H} & $D^2H$   & WLS        & `nls`  & \decimal{24\ 797}{53706}\\ %
\ref{fnlmD2H} & $D^2H$   & ML         & `nlme` & \decimal{69}{24482}     \\ %
\ref{fnlsDH}  & $D$, $H$ & WLS        & `nls`  & \decimal{24\ 802}{91248}\\ %
\ref{fnlmDH}  & $D$, $H$ & ML         & `nlme` & \decimal{76}{80204}     \\ %
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
\item red line \@ref(exr:rllnBvD) or \ref{flnDpol}: $\ln(B)=-\decimal{8}{42722}+\decimal{2}{36104}\ln(D)$
\item red line \@ref(exr:rllnBvD2H): $\ln(B)=-\decimal{8}{99427}+\decimal{0}{87238}\ln(D^2H)$
\item red line \@ref(exr:flnDlnH): $\ln(B)=-\decimal{8}{9050}+\decimal{1}{8654}\ln(D)+\decimal{0}{7083}\ln(H)$
\item red line \@ref(exr:selvar): $\ln(B)=-\decimal{6}{50202}+\decimal{0}{23756}[\ln(D)]^2+\decimal{1}{01874}\ln(H)$
\end{itemize}
...
Red             & Entry    & Fitting    & Command     & AIC                \\ %
line            &          & method     & R           &                    \\\hline%
\ref{rllnBvD}   & $D$      & OLS        & `lm` & \decimal{56}{97923}\\ %
\ref{rllnBvD2H} & $D^2H$   & OLS        & `lm` & \decimal{46}{87780}\\ %
\ref{flnDlnH}   & $D$, $H$ & OLS        & `lm` & \decimal{48}{21367}\\ %
\ref{selvar}    & $D$, $H$ & OLS        & `lm` & \decimal{45}{96998}\\ %
\hline
\end{tabular}
\end{center}
\end{table}
