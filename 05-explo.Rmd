...
```{r, eval=F, echo=T}
with(dat,plot(
  dbh^2 * heig, Btot, xlab = "D2H (cm2m)", ylab = "Biomass (t)", log = "xy", 
  pch = i, col = i
  ))
```
...
```{r, echo=T, eval=F}
with(dat, plot(
  dbh^2 * heig, Btot, log = "xy", xlab = "D2H (cm2.m)", ylab = "Biomass (t)"
  ))
```
...
kab <- display_table(.latex_scaling = "scale_down") %>%
  pack_rows(
    index = c(
...
if (book_format == "html") kab %>% scroll_box(height = "500px", extra_css = "margin-bottom: 1.2em;") else kab
